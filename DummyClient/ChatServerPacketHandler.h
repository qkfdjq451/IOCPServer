...
enum : uint16
{
	PKT_C_CHATMESSAGE = 1000,
	PKT_S_CHATMESSAGE = 1001,
};
...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_CHATMESSAGE] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::S_CHATMESSAGE>(Handle_S_CHATMESSAGE, session, buffer, len); };
	}
